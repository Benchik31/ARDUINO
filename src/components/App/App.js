...
function App(props) {
  ...
  const handleFindSaved = (word, isShort) => {
    ...
    let found = savedMovies.filter((v) =>
      v.nameRU.toLowerCase().includes(word)
    );
    if (isShort) {
      ...
      return (
        <>
          ...
          <Header1
            ...
            asideClick={handleAsideChange}
            savedLink="/saved-movies"
            moviesLink="/movies"
          />
          <Main />
          <Footer />
    ...
    if (filmCounter.length !== 0) {
      return (
        <>
          ...
          />
          <Movies
            changeShort={handleCheckbox}
            savedCards={savedMovies}
            MoreVisible={moreVisible}
            moreClick={handleMoreClick}
            searchClick={handleFindFilm}
            onClick={handleCardClick}
            cards={filmCounter}
            buttonClass="element__like"
          />
          <Footer />
        </>
      );
    ...
    if (searchSaved.length === 0) {
      return (
        <>
          ...
          <MoviesSaved
            message={movieMesSaved}
            savedCards={savedMovies}
            searchClick={handleFindSaved}
            MoreVisible={false}
            moreClick={handleMoreClick}
            onClick={handleCardClick}
            cards={savedMovies}
            buttonClass="element__saved"
          />
    ...
    return (
      <>
        <Profile
          ...
          handleAsideChange={handleAsideChange}
        ></Profile>
      </>
    );
  };
      ...
      <div className="App">
        <div className="page">
          ...
          <Switch>
            ...
            <Route>
              {isLogged ? <Redirect to="/movies" /> : <Redirect to="/" />}
            </Route>
          </Switch>
        </div>
      </div>
